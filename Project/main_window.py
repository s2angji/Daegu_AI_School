...
class WelcomeHomeWidget(QWidget):
    ...
    def __init__(self, main):
        ...
        btn = QPushButton()
        btn.setFixedHeight(QDesktopWidget().availableGeometry().height() * 0.8)
...
class MainWindow(QMainWindow):
    ...
    def insert_func_widget(self, widget):
        ...
        if self.active_widget is not None:
            self.remove_widget(self.active_widget)
        self.active_widget = widget(self)
        # 아래위로 Stretch가 적용되어 있어서 강제로 높이를 늘리기
        self.active_widget.setFixedHeight(QDesktopWidget().availableGeometry().height() * 0.8)
        self.active_widget.setContentsMargins(85, 0, 85, 0)
        self.contentVBox.insertWidget(2, self.active_widget)
    ...
    def __init__(self):
        ...
        self.resize(rect.width() * 0.8, rect.height() * 0.85)
